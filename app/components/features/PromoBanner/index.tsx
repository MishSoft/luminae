...
export default function PromoBanner({ className, image, title, description, link, bgColor }: PromoBannerProps) {
  return (
    <div className={`max-w-155 w-full hidden h-95.75  lg:flex items-center justify-between  ${className}`}
    style={{backgroundColor: bgColor}}
    >
      <div className='flex flex-col gap-7.75 text-text-on-dark px-5'>
      ...
      <div className='relative max-w-77.5 w-full h-95.75'>
        <Image fill src={image} alt='' className='object-cover xl:object-cover lg:object-center' />
      </div>
    </div>
  )
}
